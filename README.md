...
<img src="rca_ss/layout.png" alt="Final Magic GDS Layout" width="800px">
...
A **Ripple Carry Adder (RCA)** serves as a fundamental combinational block where the carry propagates sequentially through each full adder stage. While architecturally simple, executing its physical implementation through an ASIC backend exposes critical design trade-offs regarding delay optimization, power ring integrity, and area allocation.
...
The behavioral logic of the 4-bit adder was validated using a comprehensive testbench. Waveform inspection confirms flawless sum and carry-out transitions across varied test vectors with zero delay overhead in behavioral simulation.

<p align="center">
  <img src="rca_ss/rca_output_waveforms.png" width="90%" alt="GTKWave Verification Waveforms">
</p>

### 2️⃣ Logic Synthesis (Yosys)
The Verilog source description is translated into structural gates. Cells are cleanly mapped to the specific standard cells of the `sky130_fd_sc_hd` (high density) library to optimize total cell area while meeting performance targets.
...
<p align="center">
  <img src="rca_ss/floorplan.png" width="80%" alt="OpenROAD Floorplan Window">
</p>

### 4️⃣ Global & Detailed Placement
Standard cells are legally localized into the core rows. The placement tool evaluates cell congestion and minimizes initial wire length to prevent localized routing density bottlenecks.

<p align="center">
  <img src="rca_ss/placement.png" width="49%" alt="Global Placement Overview">
  <img src="rca_ss/placement_gates.png" width="49%" alt="Detailed Standard Cell Instances View">
</p>
...
<p align="center">
  <img src="rca_ss/routing.png" width="80%" alt="Routed Netlist OpenROAD GUI View">
</p>

### 6️⃣ Physical Signoff & Manufacturing Verification (DRC/LVS)
The finished layout was exported to Magic for strict physical layout checking. The design achieved flawless verification closure, yielding clean **0 DRC** anomalies, completely coherent **LVS match** alignment, and **0 Antenna** violations.

<p align="center">
  <img src="rca_ss/report.png" width="90%" alt="Manufacturability Summary Report Log">
</p>
...
```text
├── rca_ss/              # Verified visual logs, waves, and layout screenshots
...
├── rca_4bit.gds         # Exported binary stream format for foundry fabrication
...
```
./flow.tcl -design rca_4bit
```
## 🤝 Acknowledgments

 Google / SkyWater Foundation: For lowering the barrier of entry by open-sourcing the 130nm PDK.

 The OpenROAD Project: For developing robust, fully automated EDA placement, routing, and physical abstraction engines.
